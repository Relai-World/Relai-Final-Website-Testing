...
export default function AllPropertiesPage() {
  ...
  const formatBudgetRange = (property: any): string => {
    ...
    // Check for BaseProjectPrice in configurations
    let baseProjectPrice = propertyData.BaseProjectPrice;
    if (!baseProjectPrice && propertyData.configurations && Array.isArray(propertyData.configurations)) {
      for (const config of propertyData.configurations) {
        if (config && config.BaseProjectPrice) {
    ...
    if (baseProjectPrice) {
      return formatPriceValue(baseProjectPrice) || "Price on Request";
    }
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => {
      ...
      const endpoint = `${API_BASE_URL_PROPERTIES}/api/all-properties-complete`;
              ...
              <div key={propertyData.id || propertyData._id} className="h-full cursor-pointer" onClick={() => handleCardClick(propertyData.id || propertyData._id)}>
                <Card className="h-full flex flex-col overflow-hidden border-gray-200 group-hover:shadow-xl group-hover:border-blue-300 transition-all duration-300">
                  ...
                  <CardContent className="flex-1 flex flex-col p-4">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 border-t border-b border-gray-100 py-3 mb-auto">
                      ...
                      {/* <div className="flex flex-col gap-1">
                        <span className="text-xs text-gray-400 flex items-center gap-1"><Wallet size={12}/>Base Price</span>
                        <span className="text-sm font-semibold text-gray-700">
                          {(() => {
                            // Check for BaseProjectPrice in multiple locations
                            let basePrice = propertyData.BaseProjectPrice;
                            
                            // If not found directly, check in configurations array
                            if (!basePrice && propertyData.configurations && Array.isArray(propertyData.configurations)) {
                              for (const config of propertyData.configurations) {
                                if (config && config.BaseProjectPrice) {
                                  basePrice = config.BaseProjectPrice;
                                  break;
                                }
                              }
                            }
                            ...
                            if (basePrice && typeof basePrice === 'number') {
                              if (basePrice >= 10000000) {
                                return `₹${(basePrice / 10000000).toFixed(2)} Cr`;
                              } else if (basePrice >= 100000) {
                                return `₹${(basePrice / 100000).toFixed(2)} Lac`;
                              }
                      ...
                      {/* <div className="flex flex-col gap-1">
                        <span className="text-xs text-gray-400 flex items-center gap-1"><Target size={12}/>Area Name</span>
                        <span className="text-sm font-semibold text-gray-700">
                          {propertyData.AreaName || propertyData.Area || propertyData.Location || propertyData.location || 'N/A'}
                        </span>
                      </div> */}
                    </div>
                  </CardContent>
                  <CardFooter className="px-4 pt-3 pb-4 bg-gray-50 border-t mt-auto">
                    <div className="flex justify-between items-end w-full">
                      ...
                      <div className="text-center">
                        <p className="text-xs text-gray-500">Price Sheet</p>
                        <p className="text-lg font-bold text-blue-600">
                          {propertyData.PriceSheet ? propertyData.PriceSheet : 'N/A'}
                        </p>
                      </div>

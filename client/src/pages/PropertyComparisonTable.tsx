...
export default function PropertyComparisonTable() {
  ...
  // Fetch all properties for selection from MongoDB
  ...
  const availableProperties = propertiesArray.filter(
    (prop: any) => !selectedProperties.find(selected => (selected.id || selected._id) === (prop.id || prop._id))
  ).filter((prop: any) => 
    searchTerm === '' || 
    (prop.ProjectName || prop.projectName || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
    (prop.AreaName || prop.Area || prop.Location || prop.location || '').toLowerCase().includes(searchTerm.toLowerCase())
  );

  // Debug: Log properties data
  console.log('Total properties from MongoDB:', propertiesArray.length);
  ...
  const removePropertyFromComparison = (propertyId: string | number) => {
    setSelectedProperties(selectedProperties.filter(prop => (prop.id || prop._id) !== propertyId));
  };
  ...
  return (
    <Container className="pt-16 pb-8 px-4 md:px-6">
      {/* Enhanced Header */}
      <div className="bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-700 rounded-2xl p-4 md:p-8 mb-6 md:mb-8 text-white shadow-xl mt-4">
        <div className="flex items-start md:items-center gap-3 md:gap-4 mb-4">
          <Link href="/properties" className="text-white hover:text-blue-200 transition-colors mt-1 md:mt-0">
            <ArrowLeft className="w-5 h-5 md:w-6 md:h-6" />
          </Link>
          <div className="flex-1 min-w-0">
            <h1 className="text-xl md:text-4xl font-bold mb-1 md:mb-2 leading-tight">Property Comparison Hub</h1>
            <p className="text-blue-100 text-sm md:text-lg">Compare properties side-by-side to make informed decisions</p>
          </div>
          <div className="text-right flex-shrink-0">
            <Badge className="bg-white/20 text-white border-white/30 text-sm md:text-lg px-2 md:px-4 py-1 md:py-2">
        ...
        {selectedProperties.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:flex lg:gap-4 gap-3 mt-4 md:mt-6">
            {selectedProperties.map((property, index) => (
              <div key={property.id || property._id} className="bg-white/10 backdrop-blur-sm rounded-lg p-3 lg:flex-1">
                <div className="text-xs md:text-sm font-medium text-blue-100">Property {index + 1}</div>
      ...
      {selectedProperties.length < 3 && (
        <div className="mb-6 md:mb-8 bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl p-4 md:p-6 border border-gray-200 shadow-sm">
          <h2 className="text-base md:text-lg font-semibold mb-3 md:mb-4">Add Properties to Compare</h2>
        ...
        {isLoading && (
          <div className="text-center py-4">
            <BrandLoader size="md" text="Loading properties..." />
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 max-h-96 overflow-y-auto">
          {!isLoading && availableProperties.map((property: any) => (
              <div key={property.id || property._id} className="bg-white rounded-lg border border-gray-200 p-3 md:p-4 hover:shadow-md transition-shadow">
      ...
      {/* Mobile-Responsive Property Comparison */}
      {selectedProperties.length > 0 && (
        <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden mt-8">
          ...
          <div className="bg-gray-50 p-4 md:p-6 border-b border-gray-200">
            <h2 className="text-lg md:text-xl font-semibold text-gray-900 mb-4">COMPARE PROPERTIES</h2>
            
            {/* Mobile View: Horizontal Scroll Cards */}
            <div className="block md:hidden">
              <div className="flex gap-3 overflow-x-auto pb-4 snap-x snap-mandatory">
                {selectedProperties.map((property) => (
                  <div key={property.id || property._id} className="bg-white rounded-lg border border-gray-200 p-3 relative min-w-[200px] snap-start">
                    <button
                      onClick={() => removePropertyFromComparison(property.id || property._id)}
                    ...
                    <div className="text-center">
                      ...
                      <h3 className="font-semibold text-gray-900 text-xs mb-1 line-clamp-2 min-h-[2rem]">
                        {(property.ProjectName || property.projectName || property.name || '').toUpperCase()}
                      </h3>
            ...
            <div className="hidden md:grid gap-4" style={{ gridTemplateColumns: `280px repeat(${selectedProperties.length}, 1fr)` }}>
              <div className="flex items-end pb-2">
                <span className="text-gray-600 font-medium">Property Parameters</span>
              </div>
              
              {selectedProperties.map((property) => (
                <div key={property.id || property._id} className="bg-white rounded-lg border border-gray-200 p-4 relative">
                  <button
                    onClick={() => removePropertyFromComparison(property.id || property._id)}
                    className="absolute top-2 right-2 text-gray-400 hover:text-red-500 p-1 rounded-full hover:bg-gray-100 transition-colors"
                  >
          ...
          <div className="block md:hidden divide-y divide-gray-200">
            {selectedProperties.map((property, index) => {
              // Calculate price data for mobile display
              ...
              return (
                <div key={property.id || property._id} className="p-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center py-2 border-b border-gray-100">
                      <div className="flex items-center">
                        ...
                        <span className="text-sm font-medium text-gray-700">Price/sq ft</span>
                      </div>
                      <span className="text-sm font-semibold text-blue-600">{priceSqftDisplay}</span>
                    </div>
                    ...
                    <div className="flex justify-between items-center py-2 border-b border-gray-100">
                      ...
                      <span className="text-sm font-semibold text-gray-700">{property.AreaName || property.Area || property.location || 'N/A'}</span>
                    </div>
                    ...
                    <div className="flex justify-between items-center py-2 border-b border-gray-100">
                      ...
                      <span className="text-sm font-semibold text-gray-700">{property.RERA_Number || property.reraNumber || 'N/A'}</span>
                    </div>
                    ...
                    <div className="flex justify-between items-center py-2">
                      ...
                      <span className="text-sm font-semibold text-yellow-600">{property.rating || property.google_place_rating || 'N/A'}</span>
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="hidden md:block divide-y divide-gray-200">
            ...
            <div className="grid items-center py-6 px-6 bg-gradient-to-r from-blue-50 to-indigo-50" style={{ gridTemplateColumns: `280px repeat(${selectedProperties.length}, 1fr)` }}>
              ...
              {selectedProperties.map((property, index) => {
                ...
                return (
                  <div key={property.id || property._id} className={`text-center p-4 rounded-lg transition-all ${
            ...
            <div className="grid items-center py-6 px-6 bg-gradient-to-r from-blue-50 to-indigo-50" style={{ gridTemplateColumns: `280px repeat(${selectedProperties.length}, 1fr)` }}>
              ...
              {selectedProperties.map((property, index) => {
                ...
                return (
                  <div key={property.id || property._id} className="text-center p-3">
            ...
            <div className="grid items-center py-5 px-6 bg-gradient-to-r from-purple-50 to-pink-50" style={{ gridTemplateColumns: `280px repeat(${selectedProperties.length}, 1fr)` }}>
              ...
              {selectedProperties.map((property) => (
                <div key={property.id || property._id} className="text-center p-3 bg-white rounded-lg border border-gray-200 shadow-sm">
            ...
            <div className="grid items-center py-5 px-6 bg-gradient-to-r from-pink-50 to-rose-50" style={{ gridTemplateColumns: `280px repeat(${selectedProperties.length}, 1fr)` }}>
              ...
              {selectedProperties.map((property, index) => {
                ...
                return (
                  <div key={property.id || property._id} className={`text-center p-3 rounded-lg transition-all ${
            ...
            <div className="grid items-center py-6 px-6" style={{ gridTemplateColumns: `280px repeat(${selectedProperties.length}, 1fr)` }}>
              ...
              {selectedProperties.map((property) => (
                <div key={property.id || property._id} className="text-center space-y-2 px-2">
                  <button 
